...
const AnalyticsCards = () => {
    ...
    const { data, isLoading } = useQuery({
        queryKey: ["employees-analytics", currentPage],
        queryFn: () => getEmployeesAnalytics(currentPage),
    });
                ...
                {totalPages > 1 &&
                    Array.from({ length: totalPages }, (_, i) => i + 1).map((pageNum) => (
                        <button
                            ...
                            className={`w-8 h-8 rounded-md text-sm font-medium ${pageNum === currentPage ? "bg-black text-white" : "bg-transparent text-black"
