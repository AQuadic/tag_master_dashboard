...
const SignInForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Cookies.set("profile_id", response.user.profile[0].id, { path: "/" });

      if (rememberMe) {
        Cookies.set("Tag_master_admin", response.token, {
          expires: 7,
          path: "/",
        });
      } else {
  ...
  return (
    <section className="md:w-[637px] w-full md:h-[636px] bg-[#FBFBFB] rounded-2xl md:py-16 py-4 px-4">
      <div className="flex flex-col items-center">
        ...
        <h1 className="text-[#000000] text-[25px] font-bold mt-8">
          تسجيل الدخول
        </h1>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col items-center">
          ...
          <div className="mt-3 flex md:gap-56 gap-32">
            <Link to="/forgetpassword" className="text-[#003758] text-sm">
              نسيت كلمة المرور
            </Link>
            <div className="flex items-center mb-4">
              <label
                htmlFor="rememberMe"
                className="mx-2 text-sm font-normal text-[#4A4A4A]"
              >
                تذكرني
              </label>
              <input
          ...
          <button
            type="submit"
            className="md:w-[400px] w-full h-14 bg-[#003758] rounded-[12px] mt-6 text-[#FFFFFF] text-lg cursor-pointer"
          >
            تسجيل الدخول
          </button>
        </form>
        <div className="md:w-[400px] text-right mt-8">
          <Link to="" className="text-[#003758] text-sm">
            شروط الخصوصية
          </Link>
        </div>
      </div>
